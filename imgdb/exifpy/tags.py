"""
Tag definitions for EXIF metadata parsing.
...
# Tags to ignore when quick-processing (detailed=False)
...
def make_string_uc(seq) -> str:
    ...
    if not isinstance(seq, str) and make_string(seq[:8]).upper() in ('ASCII', 'UNICODE', 'JIS', ''):
        seq = seq[8:]
