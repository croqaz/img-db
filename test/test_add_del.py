...
def test_add(temp_dir):
    ...
    # test limit option
    add_op(['test/pics'], Config(db=dbname, limit=1, force=True))
    db = ImgDB(dbname)
    assert len(db) == 1
...
def test_archive(temp_dir):
    ...
    add_op(['test/pics'], Config(output=archive, operation='link', sym_links=True, shuffle=True, db=dbname))
...
def test_dry_run_archive(temp_dir):
    ...
    archive = Path(f'{temp_dir}/archive')
    add_op(['test/pics/'], Config(output=archive, db=dbname, dry_run=True))
...
def test_delete(temp_dir):
    ...
    del_op(None, Config(output=archive, db=dbname, filter='maker-model ~ BenQ'))
    assert len(listdir(archive)) == 2
    del_op(['96b26e8b05a756dbf061b1aef4ddaf62aeeec467dd6f3d65'], Config(output=archive, db=dbname))
...
def test_dry_run_delete(temp_dir):
    ...
    with pytest.raises(ValueError):
        del_op([], Config(output=archive, db=dbname))

    # A bad filter that doesn't match anything
    del_op([], Config(output=archive, db=dbname, filter='maker-model ~ ABCD'))
    assert len(listdir(archive)) == len(IMGS)

    # Dry run delete all images
    del_op([], Config(output=archive, db=dbname, filter='width > 100', dry_run=True))
    assert len(listdir(archive)) == len(IMGS)
